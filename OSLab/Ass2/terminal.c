...
int numBuiltins();
int shellCd(char **args, int numArgs);
int shellExit(char **args, int numArgs);
int shellHelp(char **args, int numArgs);
int shellHistory(char **args, int numArgs);
int shellMultiWatch(char **args, int numArgs);
...
int commandsExecute(char **args, int numArgs, int fdIn, int fdOut)
{
    ...
    for (int i = 0; i < numBuiltins(); i++)
    {
        ...
        {
            return (*builtInFuncs[i])(args, numArgs);
        }
    }
...
void ctrlRHandler()
{
    ...
    // printf("%s", commToSearch);
    // commToSearch[comLen] = '\0';
    // char *comCopy = (char *)malloc(sizeof(char) * LINE_BUFSIZE);
    // strcpy(comCopy, commToSearch);
    // comCopy[strlen(comCopy)] = '\0';
    int maxMatch = 0;
    int indexMaxMatch = 0;
    ...
    for (int i = history_index - 1; i >= history_index - MAX_HIST_SIZE; --i)
    {
        if (i == -1)
            break;
        // printf("%s\n", history[i]);
        char *histCopy = strdup(history[i]);
        histCopy[strlen(histCopy) - 1] = '\0';
        // strcpy(histCopy, history[i]);
        // histCopy[strlen(history[i])] = '\0';
        if ((strcmp(commToSearch, histCopy) == 0)) {
            printf("%s", history[i]);
            return;
        }
        int matchLen = LCSubStr(history[i], commToSearch, strlen(history[i]), comLen);
        if (matchLen < strlen(commToSearch))    continue;
        // if (matchLen == (strlen(history[i])) && matchLen > 2)
        // {
        //     printf("%s", history[i]);
        //     return;
        // }
...
int shellMultiWatch(char **args, int numArgs) {
    // printf("inside multiwatch\n");
    int xstatus;
    char **multiWatchCmds = (char **)malloc(sizeof(char *) * BUFF2D);
    int multiWatchCount = 0;
    for (int i = 2; i < numArgs; i += 2) {
        multiWatchCount++;
        multiWatchCmds[i / 2 - 1] = strdup(args[i]);
    }
    // printf("%s\n%s", multiWatchCmds[0], multiWatchCmds[1]);

    int fds[multiWatchCount], readFds[multiWatchCount];
    inotifyFd = inotify_init();
    flagnotify = 1;
    int itoFD[multiWatchCount];

    for (int i = 0; i < multiWatchCount; i++) {
        char *fileName = (char *)malloc(LINE_BUFSIZE * sizeof(char));

        sprintf(fileName, ".temp.PID%d.txt", i + 1);
    ...
    pid_t *watch_pids = (pid_t *)malloc(multiWatchCount * sizeof(pid_t));

    for (int i = 0; i < multiWatchCount; ++i) {
        // char *fileName = (char *)malloc(LINE_BUFSIZE * sizeof(char));
        // sprintf(fileName, ".temp.PID%d.txt", i + 1);
        int token_count;
        char **cmds = tokeniseLine(multiWatchCmds[i], &token_count);
        cmds[token_count] = NULL;
        // printf("%s %d %s\n", cmds[0], token_count, fileName);

        watch_pids[i] = fork();
        if (watch_pids[i] < 0) {
            printf("Could not create process. Exitting\n");
            exit(0);
        }
        if (watch_pids[i] == 0) {
    ...
    while (1) {
        ...
        while (i < len) {
            ...
            if (event->mask & IN_MODIFY) {
                ...
                printf("\"%s\" , current_time: %lu.\n", multiWatchCmds[fileIndex],
                ...
                // int curReadFd = open(fileName, O_RDONLY);
